...
export function getElementVal(name,value,domid,domid22) {  
    var domid2=getElement(name,domid,domid22)
    if (!domid2)
        return undefined;
...
export function sortfunction(a,b) {
 ...
 var orglocation=undefined //=window.parent.location; // is changed with pushstate // take the toplevel in case running in frame
 export function GetURLParam(id) {
     console.log(`In GetURLParam ${orglocation}`)
	 if (orglocation) {
			let params = (new URL(orglocation)).searchParams;
  ...
  window.addEventListener('message', event => {
     ...
     if (event.origin.includes('koios') || event.origin.includes('gpersoon')) { 
