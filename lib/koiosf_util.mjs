...
import {SetAllEventHandlers,SwitchPage,PrepLazy} from 'https://gpersoon.com/koios/lib/genhtml/startgen.mjs'
...
export async function FetchWithTimeout(url) {
	...
	var fetchresult;
	const timeoutId = setTimeout(() => { console.log(`timeout ${url}`);controller.abort();}, 15000)
	try {
...
export async function FetchIPFS(cid) {
    ...
    for (var i=0;i<ipfsproviders.length;i++) {
		...
		fetchresult=await FetchWithTimeout(url)
